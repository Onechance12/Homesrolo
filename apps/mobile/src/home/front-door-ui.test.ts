...
test('the homeowner app is chat-first with four stable destinations', () => {
  ...
  const visible = [...tabs.matchAll(/<Tabs\.Screen\s+name="([^"]+)"\s+options=\{\{ title: '([^']+)'/g)]
...
test('the shared phone chrome preserves iPhone safe areas and readable Rolodex tabs', () => {
  ...
  const workDetail = read('app/home/[homeId]/work/[projectRef].tsx')
  ...
  assert.match(workRolo, /auth\.kind === 'loading'\) return <Page><Loading \/><\/Page>/)
  assert.match(workDetail, /auth\.kind === 'loading'\) return <Page><Loading \/><\/Page>/)
  assert.match(workDetail, /resource\.state\.kind === 'loading'[\s\S]*?<Page><Loading label="Opening this work record…" \/><\/Page>/)
...
test('Rolo keeps contractor discovery on a clean chat front door', () => {
  ...
  assert.doesNotMatch(rolo, /Already in motion|activeWork|workRecordCard\(activeWork/)
...
test('new Rolo work immediately becomes the same card shown everywhere else', () => {
  const rolo = read('app/home/[homeId]/rolo.tsx')
  const saveProposal = rolo.match(/async function saveProposal\(\)[\s\S]*?\n  function startFreshConversation/)?.[0]

  assert.ok(saveProposal)
  assert.match(saveProposal, /api\.createWork[\s\S]*setConversationProjectRef\(work\.projectRef\)[\s\S]*setProposal\(null\)/)
