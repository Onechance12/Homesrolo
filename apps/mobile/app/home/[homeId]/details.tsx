...
export default function HomeDetailsScreen() {
  ...
  const resource = useResource(useCallback(() => api.getHomeRecord(homeId), [api, homeId]), auth.kind === 'signed_in')
  ...
  if (resource.state.kind === 'error') {
    return (
      <Page>
        ...
        <Notice message="Only the current controller can open or change these private details." />
        <Button
  ...
  const profile = resource.state.value
  ...
  return (
    <Page>
      ...
      <Card>
        <TextField label="Street address" value={draft.line1} onChangeText={line1 => change({ ...draft, line1 })} autoComplete="street-address" />
        <TextField label="Unit or building" value={draft.line2} onChangeText={line2 => change({ ...draft, line2 })} placeholder="Optional" />
        <TextField label="City" value={draft.city} onChangeText={city => change({ ...draft, city })} />
        <View style={styles.fieldRow}>
          <TextField label="State" value={draft.regionCode} onChangeText={regionCode => change({ ...draft, regionCode: regionCode.toUpperCase().slice(0, 2) })} autoCapitalize="characters" style={styles.shortField} />
          <TextField label="ZIP" value={draft.postalCode} onChangeText={postalCode => change({ ...draft, postalCode })} keyboardType="number-pad" autoComplete="postal-code" style={styles.zipField} />
        </View>
      </Card>
      ...
      <Card>
        ...
        <View style={styles.chips}>{HOME_TYPES.map(type => (
          <Chip key={type} label={HOME_TYPE_LABEL[type]} selected={draft.homeType === type} onPress={() => change({ ...draft, homeType: type })} />
        ))}</View>
        <TextField label="Year built" value={draft.yearBuilt} onChangeText={yearBuilt => change({ ...draft, yearBuilt, yearBuiltApproximate: yearBuilt ? draft.yearBuiltApproximate : false })} keyboardType="number-pad" placeholder="Leave blank if unknown" />
        {draft.yearBuilt ? (
          <Chip label={draft.yearBuiltApproximate ? 'Approximate year' : 'Exact year'} selected={draft.yearBuiltApproximate} onPress={() => change({ ...draft, yearBuiltApproximate: !draft.yearBuiltApproximate })} />
        ) : null}
      </Card>
        ...
        return (
          <Card key={kind}>
            ...
            <View style={styles.chips}>
              <Chip label="Here" selected={system.present === 'yes'} onPress={() => updateSystem(kind, { present: 'yes' })} />
              <Chip label="Not sure" selected={system.present === 'unknown'} onPress={() => updateSystem(kind, { present: 'unknown', year: '', approximate: false })} />
              <Chip label="Not in home" selected={system.present === 'no'} onPress={() => updateSystem(kind, { present: 'no', year: '', approximate: false })} />
            </View>
            {system.present === 'yes' ? (
              <>
                <TextField label="Installed or replaced" value={system.year} onChangeText={year => updateSystem(kind, { year, approximate: year ? system.approximate : false })} keyboardType="number-pad" placeholder="Year, if known" />
                {system.year ? <Chip label={system.approximate ? 'Approximate' : 'Exact'} selected={system.approximate} onPress={() => updateSystem(kind, { approximate: !system.approximate })} /> : null}
              </>
            ) : null}
          </Card>
        )
      ...
      <Button label={saving ? 'Saving…' : 'Save home details'} accessibilityHint="Saves these private facts to this home" icon="checkmark" disabled={saving} onPress={() => void save()} />
