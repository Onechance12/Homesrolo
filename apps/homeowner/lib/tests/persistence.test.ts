...
test('project quote migration is private, exact-project, receipt-backed, and revision safe', () => {
  ...
  assert.match(migration, /foreign key \(project_ref, home_ref\)[\s\S]+homesrolo_homeowner_projects\(project_ref, home_ref\)/i)
  assert.match(migration, /foreign key \(artifact_ref, home_ref, project_ref\)[\s\S]+homesrolo_homeowner_artifacts\(artifact_ref, home_ref, project_ref\)/i)
  assert.match(migration, /role = 'workspace_controller'/i)
