...
test('public trust pages state the current account and photo boundary', () => {
  ...
  assert.match(security, /six-digit code that you type into the same app/)
  assert.match(privacy, /web app uses a Secure, HttpOnly cookie scoped to app\.homesrolo\.com/)
  assert.match(privacy, /page code cannot read the session handle/)
  assert.match(security, /token exposed to page code/)
  assert.match(privacy, /never placed in a URL/)
