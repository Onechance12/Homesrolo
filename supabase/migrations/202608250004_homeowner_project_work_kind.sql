...
-- Keep every flavor of work in the existing project workspace. The default
-- backfills old rows and preserves old create RPCs that do not name the new
-- discriminator.
alter table public.homesrolo_homeowner_projects
  add column if not exists work_kind text not null default 'project';
...
alter table public.homesrolo_homeowner_projects
  ...
  check (work_kind in ('project', 'issue', 'repair', 'service', 'incident'));

-- This overload adds work_kind while leaving the prior signature callable by
-- a client during a rolling deploy. The table default makes that old signature
-- canonicalize to project.
...
revoke all on function public.homesrolo_create_homeowner_project(
  text, text, text, integer, text, text, text, text, text, text, text, date, text, timestamptz
) from public, anon, authenticated;
grant execute on function public.homesrolo_create_homeowner_project(
  text, text, text, integer, text, text, text, text, text, text, text, date, text, timestamptz
) to service_role;

-- The new update overload follows the same rolling-deploy rule: old callers
-- keep the existing kind, while new callers may change only this discriminator
-- under the same revision and receipt protections as every other project edit.
